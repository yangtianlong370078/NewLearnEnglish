...
; (function (window, factory) {
    ...
    jet.extend(jeDatePick.prototype, {
        ...
        renderDate: function () {
            ...
            //匹配任务
            $('.monthleft tbody tr td').each(function (index) {
                let ym = $(this).attr("ym");
                let array = ym.split('-');
                let y = parseInt(array[0]);
                let m = parseInt(array[1]);

                let result = categorydatas.find(item => item.year === y && item.month === m);

                let value2 = 0;
                let Count = 0;    //学习数量
                let taxkcount = 0;//任务数量
                let datedaytype = 0;//0为当前月；-1为已过去时间；1为还未到时间
                if (result != undefined) {
                    Count = result.totalcount;
                    let totalcount = result.totalcount;

                    if (result.task != null && result.task != undefined) {
                        taxkcount = result.task.count;
                    }
                    if (taxkcount > 0) {
                        value2 = totalcount / taxkcount * 100;
                    }
                }

                let dqdata = new Date();
                let dy = dqdata.getFullYear();
                let dm = dqdata.getMonth() + 1;

                if (dy > y || (dy == y && dm > m)) {
                    datedaytype = -1;
                } else if (dy < y || (dy == y && dm < m)) {
                    datedaytype = 1;
                }

                var value3 = Math.floor(value2);
                var typeclass = "text-mute1";
                if (taxkcount > 0) {
                    if (value3 >= 100) {
                        typeclass = "text-success1";
                    }
                    else if (value3 > 0) {
                        typeclass = "text-warning1";
                    } else if (value3 == 0) {
                        typeclass = "text-danger1";
                    }
                ...
                var textstr = "<text class='texttt " + typeclass + "'>" + value3 + "</text>"
        ...
        dateTemplate: function () {
            ...
            var lyPrev = '<em class="yearprev yprev jedatefont" @on="yearBtn(lprev,' + aowArr[0] + ')"><i class="icon-Angle-left ms-5 fs-22"><span class="path1"></span><span class="path2"></span></i></em>',
                lyNext = '<em class="yearnext ynext jedatefont" on="yearBtn(lnext,' + aowArr[2] + ')"><i class="icon-Angle-right fs-22"><span class="path1"></span><span class="path2"></span></i></em>',
                ryPrev = '<em class="yearprev yprev jedatefont" on="yearBtn(rprev,' + aowArr[3] + ')"><i class="icon-Angle-left  ms-5 fs-22"><span class="path1"></span><span class="path2"></span></i></em>',
                ryNext = '<em class="yearnext ynext jedatefont" @on="yearBtn(rnext,' + aowArr[1] + ')"><i class="icon-Angle-right fs-22"><span class="path1"></span><span class="path2"></span></i></em>',
        ...
        methodEventBind: function () {
            ...
            jet.extend(that, {
                ...
                nowBtn: function () {
                    ...
                    let newrq = parseCustomDate(dateVal, 0);
                ...
                sureBtn: function () {
                    ...
                    var newrq = parseCustomDate(dateVal, 0);
                    window.CalendarApp.$calendarObj.fullCalendar('gotoDate', newrq);
                    //触发事件
                    settjsuju(newrq);
                },
