...
namespace LearnEnglish.Infrastructure.Repositories
{
    ...
    public class WordQueryRepository : DapperRepository<object>, IWordQueryRepository
    {
        ...
        /// <inheritdoc/>
        public async Task<(IEnumerable<ShowTranslateDto> items, int total, int newCount, int learningCount, int masteredCount)> GetCollectedWordsPagedAsync(
            int userId, int status, string? name, int pageIndex, int pageSize, string orderBy)
        {
            ...
            var statusCountSql = $@"SELECT 
                ...
                FROM mylexicon t4
                INNER JOIN lexicon t2 ON t4.lexiconId = t2.id
                WHERE t4.userId = @UserId AND t4.iscollect = 1";
            ...
            var statusCounts = await connection.QueryFirstOrDefaultAsync<(int NewCount, int LearningCount, int MasteredCount)>(
                statusCountSql, new { UserId = userId, Name = name });
